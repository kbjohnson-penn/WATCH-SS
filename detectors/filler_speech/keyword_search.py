...
class FillerKeywordDetector(KeywordDetector):
    ...
    def detect(self, text):
        ...
        if self.flag_nonwords:
            output += [(token.idx, token.idx + len(token)) for token in doc if token.is_oov and not (token.is_punct or token.is_space or token._.is_silence_tag or token._.is_inaudible_tag or token._.is_event_tag)]
