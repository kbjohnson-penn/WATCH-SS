...
def clean_CHAT_text(text):
    ...
    text = re.sub(r"\(\.{1,3}\)", "...", text)                                          # replace pause tags with ellipses
    ...
    text = re.sub(r"&(\w+)", r"\1", text)                                               # remove & prefix for phonological string (ums uhs ahs) 
...
def load_adress_CHAT_transcripts():
    ...
    transcripts["Transcript_clean"] = transcripts["Transcript"].apply(clean_CHAT_text)
    transcripts["Utterance"] = transcripts.apply(lambda x: f"{str(x.Timestamp)} {x.Speaker}: {x.Transcript_clean}", axis=1)
    return transcripts[["T_start_ms", "T_end_ms", "Timestamp", "Speaker", "Transcript", "Transcript_clean", "Utterance"]]
